...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <div class="observatory" data-view="overview">
    ...
    <main>
      ...
      <section class="detail-heading" aria-labelledby="detail-title" hidden>
        ...
        <p id="terrain-status" class="eyebrow terrain-status" role="status" hidden></p>
  ...
  <dialog id="settings-dialog" class="panel-dialog" aria-labelledby="settings-title">
    ...
    <label class="setting-row"><span>Surface relief<small>Earth shading · Moon & Mars 3D terrain</small></span><input type="checkbox" data-layer="relief" checked></label>
  ...
  <dialog id="sources-dialog" class="panel-dialog sources-dialog">
    ...
    <div class="source-entry"><h3>Orbit illumination</h3><p>Day and night follow the Sun in the J2000 orbit frame, with body tilt and rotation tied to scene time. Earth–Moon eclipse shadows use physical radii and separations, including partial solar-disk coverage in the penumbra, independently of enlarged display scales. Orbit markers are spherical; cast terrain shadows, atmospheric refraction during lunar eclipses, and reflected planetshine are omitted. A small display fill keeps dark silhouettes readable.</p></div>
    ...
    <div class="source-entry"><span class="source-tag">REFERENCE IMAGERY</span><h3>Earth, Moon & Mars imagery</h3><p>Solar System Scope / INOVE maps, based on NASA data, under CC BY 4.0. Earth assets are distributed through the Three.js Earth example. Maps were resized and converted to WebP; Moon and Mars use the same 8K Solar System Scope maps from the first frame through close-ups. Cloud coverage and night lights are reference composites; cloud drift is illustrative. Slow viewing rotation is independent of scene time. The shadow toggle offers a fully lit inspection view. Mars haze is an artistic atmospheric model.</p><a href="https://www.solarsystemscope.com/textures/" target="_blank" rel="noopener">Solar System Scope ↗</a></div>
    <div class="source-entry"><span class="source-tag">NASA / LRO</span><h3>Lunar color & elevation</h3><p>NASA Scientific Visualization Studio CGI Moon Kit. LROC color and LOLA elevation, assembled by Ernie Wright. The 2019 color map is optimized for visual rendering. The small Earth–Moon preview uses the LROC color mosaic. The observatory uses the consistent 8K imagery credited above at every zoom level. Close-up geometry uses measured LOLA elevations from the PDS archive, in meters relative to the 1,737.4 km lunar reference sphere.</p><a href="https://svs.gsfc.nasa.gov/4720/" target="_blank" rel="noopener">NASA CGI Moon Kit ↗</a></div>
    <div class="source-entry"><span class="source-tag">NASA / MGS</span><h3>Martian elevation</h3><p>Mars Orbiter Laser Altimeter MEGDR topography, version 2.0, 32 pixels per degree. Signed meter elevations are preserved when resampling and tiling. MOLA data includes interpolated gaps.</p><a href="https://pds-geosciences.wustl.edu/missions/mgs/megdr.html" target="_blank" rel="noopener">NASA PDS MOLA archive ↗</a></div>
    <div class="source-entry"><span class="source-tag">RENDERING</span><h3>Made of light and code</h3><p>Three.js WebGPU, custom TSL materials and a single-scattering atmosphere shader. The landing view composes three independent viewing frames. Relative sizes and separations are artistic; each body retains its calculated relationship to the Sun. Moon and Mars use a continuous 3D terrain surface at every zoom level, with smoothly refined geometry, streamed elevation tiles, and consistent 8K color maps. Heights retain their measured scale. Global terrain is sampled at about 1.3 km on the Moon and 2.6 km on Mars at the equator; this view does not resolve landers or individual rocks. Mars elevations are relative to the MOLA areoid and applied to the display sphere. Terrain slopes affect lighting; cast terrain shadows are not modeled.</p><a href="https://threejs.org/examples/webgpu_tsl_earth.html" target="_blank" rel="noopener">Three.js Earth reference ↗</a></div>
  </dialog>
`;
...
function navigate(view: View, body: BodyId = currentBody, history = true) {
  ...
  $('#terrain-status').hidden = view !== 'detail' || body === 'earth';
  $('#terrain-status').textContent = 'Loading 3D terrain…';
  ...
  if (history) window.history.pushState(null, '', view === 'overview' ? location.pathname + location.search : `#${view === 'orbit' || view === 'satellites' ? view + '/' : ''}${body}`);
  updateReadouts();
}
...
async function start() {
  ...
  try {
    ...
    observatory.onStats = stats => {
      ...
      terrainStatus.hidden = currentView !== 'detail' || currentBody === 'earth';
      const terrainState = $('#scene').dataset.terrain;
      terrainStatus.textContent = terrainState === 'error' ? 'Terrain unavailable · retrying'
        : terrainState !== 'ready' ? 'Loading 3D terrain…'
        : !$<HTMLInputElement>('[data-layer="relief"]').checked ? 'Surface relief off'
        : '3D terrain · true elevation scale';
    };
